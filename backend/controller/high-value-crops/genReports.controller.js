...
export const generateHVCSaMPR = async (req, res) => {
  ...
  try {
    ...
    // Fetch employee information if employeeId is provided
    ...
    if (employeeId) {
      try {
        ...
        // Continue with default name if employee fetch fails
      }
    }
    ...
    // Insert generated by and generated at rows below the date row
    const generatedAt = new Date();
    const formattedTimestamp = generatedAt.toLocaleString('en-US', { 
      year: 'numeric', 
      month: 'long', 
      day: 'numeric', 
      hour: '2-digit', 
    ...
    // Insert row 8 for "Generated by"
    worksheet.spliceRows(8, 0, []);
    worksheet.getRow(8).getCell(1).value = 'Generated by:';
    worksheet.getRow(8).getCell(2).value = employeeName;

    // Insert row 9 for "Generated at"
    worksheet.spliceRows(9, 0, []);
    worksheet.getRow(9).getCell(1).value = 'Generated at:';
    worksheet.getRow(9).getCell(2).value = formattedTimestamp;
    ...
    // Template row is now at row 13 (was 11, but we inserted 2 rows)
    const templateRow = worksheet.getRow(13);

    // Start data rows at 13 (was 11, but we inserted 2 rows)
...
export const generateHVCPR = async (req, res) => {
  ...
  try {
    ...
    // Insert generated by and generated at rows below the month header
    const generatedAt = new Date();
    const formattedTimestamp = generatedAt.toLocaleString('en-US', { 
      year: 'numeric', 
      month: 'long', 
    ...
    // Insert row 3 for "Generated by"
    worksheet.spliceRows(3, 0, []);
    worksheet.getRow(3).getCell(1).value = 'Generated by:';
    worksheet.getRow(3).getCell(2).value = employeeName;

    // Insert row 4 for "Generated at"
    worksheet.spliceRows(4, 0, []);
    worksheet.getRow(4).getCell(1).value = 'Generated at:';
    worksheet.getRow(4).getCell(2).value = formattedTimestamp;

    const UnifiedFarmerRecordModel = global.getUnifiedFarmerRecordModel(yNum);

    // Section layout based on the template:
    // Template header was at row 3, but we inserted 2 rows, so it's now at row 5
    const TEMPLATE_HEADER_TOP = 5; // header source starts at row 5 (A5:Y8) - was row 3, but we inserted 2 rows
    const HEADER_ROWS = 4; // rows 5..8 (was 3..6)
